...
options.theme = {
  ...
  js_src: options.rootPath.theme + 'src/',
...
// Build everything.
gulp.task('build', ['sass', 'drush:cc', 'lint', 'babel'])

// Default watch task.
// @todo needs to add a javascript watch task.
gulp.task('watch', ['watch:css', 'watch:babel'])
...
gulp.task('drush:cc', function() {
  ...
  return gulp
    .src('', { read: false })
    .pipe($.shell([options.drush.alias.css_js]))
})
